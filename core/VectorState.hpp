...
namespace atlas
{
    ...
    public:
        ...
        inline void setLMUL(uint8_t value)
        {
            ...
            sparta_assert(((value & (value - 1)) == 0 && (sew_ <= vlen_ / 8 * value)), "Invalid LMUL value.");
            lmul_ = value;
        }
        ...
        inline void setSEW(uint8_t value)
        {
            ...
            sparta_assert(((value & (value - 1)) == 0 && (value <= vlen_ / 8 * lmul_)), "Invalid SEW value.");
            sew_ = value;
        }
        ...
        inline uint8_t getVLMAX()
        {
            return getVLEN() / 8 * getLMUL() / getSEW();
        }

        template <typename XLEN>
        XLEN vsetAVL(AtlasState* state_ptr, bool set_max, XLEN avl = 0)
        {
        ...
        template <typename XLEN>
        void vsetVTYPE(AtlasState* state_ptr, XLEN vtype)
        {
        ...
        template <typename XLEN>
        void vsetVSTART(AtlasState* state_ptr, XLEN vstart)
        {
            setVSTART(vstart);
    ...
    template <uint8_t ElemWidth>
    struct Element
    {
        using ElemType = GetUintType<ElemWidth>;

        void setVal(ElemType value)
        {
            WRITE_VEC_ELEM<ElemType>(state_ptr, reg_id, value, idx);
        }

        ElemType getVal()
        {
            return READ_VEC_ELEM<ElemType>(state_ptr, reg_id, idx);
        }
    ...
    public:
        ...
        public:
            ...
            using value_type = Element<ElemWidth>;
            using pointer = Element<ElemWidth>*;
            using reference = Element<ElemWidth>&;
            using iterator_category = std::forward_iterator_tag;

            ElementIterator(Elements<ElemWidth>* elems_ptr, uint8_t index) :
            ...
            Element<ElemWidth>& operator*()
            {
                elems_ptr_->current_.state_ptr = elems_ptr_->state_ptr_;
                elems_ptr_->current_.reg_id = elems_ptr_->reg_id_ + index_ / (elems_ptr_->config_ptr_->getVLEN() / ElemWidth);
                elems_ptr_->current_.idx = index_ % (elems_ptr_->config_ptr_->getVLEN() / ElemWidth);
            ...
            Element<ElemWidth>& operator++()
            {
                ...
                {
                    index_ = elems_ptr_->getNextIndex_(++index_);
                }
                else
                {
                    ++index_;
            ...
            Element<ElemWidth>& operator++(int)
            {
                ++(*this);
                return *this;
            }
        ...
        private:
            Elements<ElemWidth>* elems_ptr_;
            uint8_t index_;
        }; // class ElementIterator

        Elements(AtlasState* state_ptr, uint8_t reg_id, bool masked) :
            ...
            masked_(masked)
        {
            if (masked_)
            {
                for (uint8_t idx; idx < config_ptr_->getVL() / VLEN_MIN; ++idx)
                {
                    mask_.push_back(READ_VEC_ELEM<GetUintType<VLEN_MIN>>(state_ptr, atlas::V0, idx));
                }
            }
        }
        ...
        ElementIterator begin()
        {
            if (masked_)
            {
                return ElementIterator(this, getNextIndex_(config_ptr_->getVSTART()));
            }
            else
            {
                return ElementIterator(this, config_ptr_->getVSTART());
            }
        }

        ElementIterator end()
        {
            return ElementIterator(this, config_ptr_->getVL());
        }

    private:
        uint8_t getNextIndex_(uint8_t index)
        {
